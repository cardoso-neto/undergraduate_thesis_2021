...
As novas mídias sociais são um avanço se comparado às mídias tradicionais, visto que concedem a inclusão de mais pessoas aos espaços de discussão.
...
As plataformas mais usadas de mídia social pertencem a entidades privadas com os mais variados interesses.
Elas são de uso gratuito,no entanto as corporações que detêm seu controle são de fins lucrativos e, em sua grande maioria, são empresas de capital aberto.
...
Não obstante, uma proporção muito grande (e crescente) das nossas comunicações diárias acontece através dessas plataformas.
Plataformas cujo interesse, em vez de prover a melhor experiência de usuário possível, parecem focar nos lucros.

Afim de auxiliar a  mitigar esse problema de interesses desalinhados, acreditamos que a solução seria a criação de mídias sociais inteiramente descentralizadas e que não dependam de nenhum terceiro no controle.
A tecnologia para criar plataformas deste tipo não apenas existe há anos, como já foi posta para uso no Dtube\footnote{\url{https://d.tube/}}, diaspora\footnote{\url{https://diasporafoundation.org/}}, LBRY\footnote{\url{https://lbry.com/}}, Peertube\footnote{\url{https://joinpeertube.org/}}, mastodon\footnote{\url{https://mastodon.social/about}}, scuttlebutt\footnote{\url{https://scuttlebutt.nz/}}... e uma longa lista de redes sociais com poucos usuários e pouco conteúdo.

O objetivo das mídias sociais é o compartilhamento de conteúdo gerado e curado por usuários, o \textit{network-effect}\ref{foot:net-effect} é especialmente expressivo aqui.
...
As plataformas sociais corporativas encobriram  problemas como a alta influência que as redes sociais sofrem dos poderes políticos e econômicos, já que a infraestrutura de comunicação e tecnologia é moldada por regimes regulatórios, acordos internacionais, ganancias corporativas e práticas intrusivas de vigilância. Para exemplificar essa situação citamos Sasha Costanza-Chock\cite{Censura1}:
...
% Nome dessa subseção
\subsection{Alinhamento de Interesses}
...
Desta forma, a questão de pesquisa que norteará este trabalho é definido por: 

``Como podemos transicionar as mídias sociais atuais para um modelo mais descentralizado sem perdermos os usuários e o conteúdo delas?''
...
O termo Web 3.0 foi criado pelo jornalista John Markoff, do New York Times, baseado na evolução do termo Web 2.0 criado por O'Really em 2004~\cite{Web32}.
Algumas evoluções desse momento poderão ser a ``Web Semântica'' ou a ``Web Inteligente'', mas ressaltamos que neste trabalho usamos o termo apenas no sentido de web distribuída, descentralizada, ou desintermediada, assim como ele costuma ser usado por cipherpunks e projetos de criptomoedas.
...
Investigar que elementos devem ser considerados para propor um sistema que transforme a arquitetura de software centralizada das mídias sociais da web 2.0 em uma arquitetura descentralizada baseada na web 3.0.
...
Nesta seção apresentamos os resultados das comparações entre as rede sociais analisada previamente neste capítulo, para isso utilizaremos a tabela~\ref{tab:social-media-comparison} para ilustrar a análise aqui feita e destacar elementos que julgamos importantes da análise individual feita das mídias sociais atuais.
    ...
    \begin{center}
        ...
        \resizebox{1.0\linewidth}{!}{
            \begin{tabular}
                ...
                \tabularnewline
0                Twitch                                & Anúncios intrusivos     & Centralizada             & 100M                        & Sensível
                \tabularnewline
...
Enquanto as redes sociais que apresentaram na coluna "arquiteturas" a característica de  distribuídas e utilizam algum tipo de proteção aos dados de seus usuários como o \textit{Memo} e o \textit{Bitstagram} que utilizam \textit{blockchain} para garantir a proteção e imutabilidade dos dados conseguiram apresentar na coluna sensibilidade a censura total resistência à censura. Fica destacado que quanto mais os dados estão distribuídos na rede mais difícil será  censurar ou deletá-los. Assim, por os dados estarem distribuídos pela rede exercer algum controle sobre eles fica dificultado pois um mesmo dado pode estar em vários nodos da rede, assim para exercer a mesma influência que no modelo centralizado necessita o controle de todos os nodos da rede. Outro aliado é o armazenamento de dados na \textit{blockchain}, pois armazenando os dados no formato de transações garante a autenticidade dos dados pela rede, assegurando também a imutabilidade dos dados.
...
Nas próximas seções descrevemos individualmente cada ferramenta necessária para concretizar nossa visão para uma mídia social.
...
\section{Consequências esperadas da quebra dos muros}
